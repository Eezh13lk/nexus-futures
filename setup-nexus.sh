...
<WebView xmlns:android="http://schemas.android.com/apk/res/android"
    android:id="@+id/web" android:layout_width="match_parent"
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <application android:theme="@style/Theme.NexusFutures"
        android:label="@string/app_name" android:usesCleartextTraffic="false">
...
header{padding:18px;font-size:25px;font-weight:700}.sub{padding:0 18px 16px;color:#9aa7c7}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:12px}
.card{background:#151c31;border:1px solid #283452;border-radius:14px;padding:15px}
.label{color:#91a0c4;font-size:12px}.value{font-size:21px;margin-top:7px}
...
<header>NEXUS Futures</header><div class="sub">V1 • Separate app • PAPER mode by default</div>
<div class="grid">
...
<select id="symbol"><option>BTCUSDT</option><option>ETHUSDT</option><option>BNBUSDT</option><option>SOLUSDT</option></select></div>
...
function start(){status.textContent='RUNNING';log('Paper engine started for '+symbol.value+' at '+risk.value+'% risk.')}
function stop(){status.textContent='STOPPED';log('Bot stopped. No real orders were sent.')}
function emergency(){status.textContent='STOPPED';log('Emergency action: paper positions cleared. Live trading is disabled in V1.')}
</script></body></html>
EOF
